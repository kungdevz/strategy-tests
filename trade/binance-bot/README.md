...
```bash
binance-bot$ sam build --use-container
```
...
```bash
binance-bot$ sam logs -n StockCheckerFunction --stack-name "binance-bot" --tail
```
...
```bash
binance-bot$ pip install -r tests/requirements.txt --user
# unit test
binance-bot$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
binance-bot$ AWS_SAM_STACK_NAME="binance-bot" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "binance-bot"
```
